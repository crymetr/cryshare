...
namespace ShareX
{
    public class TaskInfoStatus
    {
        ...
        public TaskInfoStatus(WorkerTask task)
        {
            Task = task;
            Update();
        }
    ...
    /// <summary>
    /// Actions on the selected task list items. Everything here is local: open, copy, edit, delete.
    /// </summary>
    public class TaskInfoManager
    {
        ...
        private void CopyTexts(IEnumerable<string> texts)
        {
            if (texts != null && texts.Count() > 0)
            {
                ...
                {
                    if (ClipboardHelpers.CopyText(text))
                    {
        ...
        public void TryCopy()
        {
            if (IsItemSelected && SelectedItem.IsFilePathValid)
            {
